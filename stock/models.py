...
class Stock(models.Model):
    ...
    current_price = models.IntegerField() # 가격은 숫자니까 IntegerField
...
class Reason(models.Model):
    ...
    report = models.ForeignKey(Recommendation, on_delete=models.CASCADE)
    sequence = models.IntegerField() # 카드 순서 (1, 2, 3...)
    category = models.CharField(max_length=50) # 재무, 뉴스, 선례 등
    content = models.TextField()
    visual_url = models.CharField(max_length=255, blank=True, null=True) # 이미지가 없을 수도 있으니 빈칸 허용
...
class Bookmark(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE) # 누가 북마크 했는지 연결
    report = models.ForeignKey(Recommendation, on_delete=models.CASCADE) # 어떤 레포트를 북마크 했는지 연결
    created_at = models.DateTimeField(auto_now_add=True)
